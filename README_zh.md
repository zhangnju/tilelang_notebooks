...
|------|------|------|:-------:|:------:|:--------:|:----------:|:---------:|
| 01 | Copy（多块并行） | `BLOCK_N=1024` | 0.0060ms | 0.0072ms | **0.0055ms** | **+9%** | **+31%** |
...
所有内核在 **AMD Radeon 8060S**（RDNA3.5 iGPU，gfx1151，20 CU，128 GB 统一内存）上完成测试。
...
|------|------|----------------|:-------:|:------:|:--------:|:----------:|:---------:|
| 01 | Copy（多块并行） | `BLOCK_N=8192` | 0.0065ms | 0.0085ms | **0.0054ms** | **+20%** | **+57%** |
...
- TileLang 在计算密集型内核上表现突出：**04_bwd**（+191%）、**07_attn**（+39%）、**09_conv**（+792%）、**10_dequant**（+64%）
...
|------|------|----------------|:-------:|:------:|:--------:|:----------:|:---------:|
| 01 | Copy（多块并行） | `BLOCK_N=2048` | 0.0040ms | 0.0088ms | **0.0037ms** | **+8%** | **+138%** |
